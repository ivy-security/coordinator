...
interface MeetingFormProps {
  ...
  onSubmit: (data: MeetingFormData, imageFile: File | null) => Promise<void>;
  submitLabel: string;
...
export default function MeetingForm({
  ...
  const [requiredEmails, setRequiredEmails] = useState<string[]>(initialData?.requiredEmails || []);
  const [optionalEmails, setOptionalEmails] = useState<string[]>(initialData?.optionalEmails || []);
  ...
  const [ranges, setRanges] = useState<RangeOption[]>(
    initialData?.ranges || [{ id: "1", date: "", startTime: "09:00", endTime: "17:00" }]
  );
